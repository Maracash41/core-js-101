...
function isCreditCardNumber(ccn) {
  const ccnArr = ccn
    ...
    .split('')
    .reverse();
  return ccnArr.reduce((acc, item, index) => {
    if ((index + 1) % 2 === 0) {
      let num = Number(item) * 2;
      if (num > 9) {
        num -= 9;
      }
      return acc + num;
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < pathes[0].length; i += 1) {
    for (let z = 1; z < pathes.length; z += 1) {
      if (pathes[0][i] !== pathes[z][i]) {
        const lastPath = path.lastIndexOf('/');
        return path.slice(0, lastPath + 1);
      }
    }
    path += pathes[0][i];
  }
...
function getMatrixProduct(m1, m2) {
  const columnA = m1[0].length;
  const columnB = m2[0].length;
  const rowA = m1.length;
  const rowB = m2.length;
  const result = [];
  if (columnA !== rowB) return false;
  for (let i = 0; i < rowA; i += 1) {
    result[i] = [];
  }
  for (let i = 0; i < rowA; i += 1) {
    for (let z = 0; z < columnB; z += 1) {
      let t = 0;
      for (let y = 0; y < rowB; y += 1) {
        t += m1[i][y] * m2[y][z];
      }
      result[i][z] = t;
    }
...
function evaluateTicTacToePosition(position) {
  const row = position.length;
  const column = position[0].length;
